...
public class WanderingIguana : MonoBehaviour
{
    ...
    void Update()
    {
        //Determine if we are headed for an obstacle (so we can decide if we need to turn)
        ...
        {
            //IS there an obstacle i the way that is close enough to warrant a turn 
            if(hit.distance < obstacleRange)
            {
                //if our turn value is not set (0), we need to decide on a left or right turn 
                if(Mathf.Approximately(turn, 0.0f))
                {
                    // flip a coin (0,1), 0 means turn left, 1 means turn right
                    turn = Random.Range(0, 2) == 0 ? -0.75f : 0.75f;
                }
                
                // Blending will cuase the iguana to move forward and turn at the same time 
                // turn quickly, move forward slowly
                Move(turn, 0.1f);
            }
            else // no obstacle, k to move forward withouth turning 
            {
                float forwardSpeed = Random.Range(0.05f, 1.0f);
                turn = 0.0f; // set the turn value to 0 (well know to pick a new direction when we next turn )
